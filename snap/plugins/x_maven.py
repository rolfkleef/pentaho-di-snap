...
class MavenPlugin(snapcraft.plugins.jdk.JdkPlugin):
    ...
    def build(self):
        ...
        for f in self.options.maven_targets:
            ...
            src = os.path.join(self.builddir, f, 'assemblies/pdi-ce/target/')
            #types = ('*.tar.gz', '*.zip', '.*.tgz', '*.war', '*.jar')
            #arfiles = []
            #for files in types:
            #    arfiles.extend(glob.glob(files))
            jarfiles = glob.glob(os.path.join(src, '*.jar'))
            warfiles = glob.glob(os.path.join(src, '*.war'))
            arfiles = glob.glob(os.path.join(src, '*.zip'))
            zipfiles = glob.glob(os.path.join(src, '*.zip'))
